...
contract AutolaunchPauseScopeTest is AutolaunchFixture {
    ...
    function test_FAC_019_PauseNeverBlocksExistingLifecycleOperations() public {
        ...
        winner.subject.transfer(outsider, 1_000e18);
        vm.stopPrank();
        vm.startPrank(outsider);
        winner.subject.approve(address(splitter), 1_000e18);
    ...
    function test_SPL_010_ClaimsAndUnstakingIgnoreTheLaunchPause() public {
        ...
        vm.prank(treasury);
        launched.subject.transfer(outsider, 2_000e18);

        // Stake while the factory is open, then pause and do everything else under the pause.
        vm.startPrank(outsider);
        launched.subject.approve(address(splitter), 2_000e18);
        splitter.stake(1_000e18);
        vm.stopPrank();
        ...
        assertEq(launched.subject.balanceOf(outsider), 2_000e18, "a paused factory blocked unstaking");
