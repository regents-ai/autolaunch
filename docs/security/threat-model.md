...
headers against a *previous* candidate's production bytecode; `regent-alv1.7` and its successor
`regent-alv1.7.1` both changed production bytes, so they await re-execution under the founder's
separate read-only authority. That provider-backed evidence is `regent-4wx`'s, it runs once, and it
runs against the final candidate — after this correction and after the separate liquidity-position
locker are both integrated — rather than once per intermediate candidate. Deployment and a signed
ceremony remain outside this packet either way.
