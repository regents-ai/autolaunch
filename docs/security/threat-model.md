...
headers against a *previous* candidate's production bytecode; `regent-alv1.7`, `regent-alv1.7.1` and
`regent-alv1.10` all changed production bytes, so they await re-execution under the founder's
...
than once per intermediate candidate. The liquidity-position locker that earlier packets named as a
...
| --- | --- | --- |
| One block prevents atomic exit, and nothing more | A stake still credits in the same block and still earns from the very next recognition, and what it earns can still be claimed in that same block. The one thing it cannot do is leave: any unstake, partial or complete, needs a later block than that account's own latest stake, and a further stake resets the delay for the account's whole position. That is enough to make a stake, recognize, claim and unstake round trip inside one transaction fail entirely, because financing such a position is only free when the principal returns in the same transaction. It is deliberately not a cooldown, an epoch, a queue, or any form of time weighting, and it is not what bounds anyone's earnings. A holder who genuinely funds SUBJECT and holds it across a block boundary is a legitimate staker and is meant to earn; what bounds what it earns is the fixed-supply fraction — its stake as a share of the complete 100 billion SUBJECT supply — so buying a larger share of the net means buying a larger share of the supply, at its price, and holding it. | `SPL-009`, `SPL-005`, `SPL-013` |
| Permissionless surplus-recognition ordering | Anyone may call `recognizeSurplusRevenue`, so the block in which a bare transfer becomes revenue is chosen by an untrusted caller, and therefore so is the stake set that shares it. A caller may still put itself into the stake set first and take the resulting share — but only its supply-coverage share of that recognition, and only by leaving the principal staked into a later block. Recognition still skims exactly once, still delivers everything the staked supply does not cover to the treasury in the same transaction, and still cannot relabel principal, unclaimed liability, or the carried remainder. | `SPL-011`, `SPL-009`, `SPL-005`, `SPL-008` |
...
| A self-made clone bound to a different-supply token is not admitted | The 100 billion denominator is a constant, not a reading of the bound token. The authentic factory and escrow graph is what proves every admitted SUBJECT carries exactly that supply, so no clamp, no `totalSupply` call and no supply check is added to the splitter. Someone who clones the implementation themselves and binds a token with a smaller supply gets a splitter that still divides by 100 billion, and therefore under-allocates to its stakers and over-delivers to its treasury; someone who binds a larger-supply token and stakes past the denominator gets recognitions that revert outright. Neither is a protocol launch, neither is reachable through the factory, and both fail in the direction that protects the protocol rather than the clone. | `SPL-005`, `DEP-060` |
