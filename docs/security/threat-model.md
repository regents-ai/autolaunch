...
mitigation below except the dependency, binding, and ABI provenance claims is still pending.
...
3. **External deployed contracts.** REGENT, USDC, the CCA factory, PoolManager,
   ...
   is unproven until the authorized fork gate runs (`DEP-030` through `DEP-035`).
...
5. **Recovery admin.** An immutable deployed contract may move only forced ETH and unsupported
   ERC20s, only to the immutable treasury (`RCV-009`, `RCV-010`).
...
| Receiver creation | any caller | `createPaymentReceiver`, receiver creation | `FAC-025`, `RCV-001` |
...
| Recovery | immutable recovery admin | `recoverUnsupportedToken`, `recoverForcedETH` | `RCV-009`, `RCV-010`, `RCV-011` |
...
| --- | --- | --- |
| CCA factory and auction | auction creation, finalization, sweeps, refunds | technical failure is an ordinary revert with no retry state (`STR-004`); admission requires the exact runtime code hash and a zero protocol fee controller (`DEP-030`, `DEP-031`) |
...
| --- | --- | --- |
| Dependency substitution | build against a moving branch, a look-alike commit, or a copied `lib/` tree without git metadata | `DEP-001`–`DEP-008` |
| Compiler or setting drift | different solc, optimizer runs, or re-enabled metadata changing deployed bytecode | `DEP-009`, `DEP-010`, `DEP-014` |
| Authority forgery in the frozen literals | edited address, code hash, or selector in a manifest, fixture, or binding source | `DEP-011`, `DEP-012` |
...
| Repeat or replay | migrating or retiring twice, re-running an initializer | `FAIL-008`, `ABI-007` |
| Hook boundary escape | unregistered key, foreign caller, hostile router, reentrancy | `HOK-017`, `HOK-018` |
| Fee lane evasion | tiny swaps, rounding, exact-output, or unspecified-currency paths avoiding a lane | `HOK-005`–`HOK-009`, `HOK-012` |
| Fee retention | hook keeping attributable inventory for a later flush | `HOK-014`, `HOK-015`, `INV-004` |
| Staker dilution or theft | interleaved stake and unstake around a deposit, or claiming another staker's share | `SPL-012`, `SPL-013`, `INV-003` |
| Remainder skimming | repeatedly recognizing dust to drain the carried remainder | `SPL-007`, `INV-005` |
| Recovery abuse | recovering a core token, or recovering to a caller-chosen destination | `RCV-009`, `RCV-010` |
| Malicious token | a token that reverts, re-enters, or lies about transfers | `SPL-014`, `RCV-011` |
| Gas exhaustion | a terminal transaction that cannot fit in a Base block | `GAS-003`–`GAS-006` |
| Evidence laundering | a mock closing a deployed-runtime claim, or a placeholder test closing a future claim | ledger evidence classes, enforced by `bin/check-requirements.py` |
