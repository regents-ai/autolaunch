...
| `DEP-070..075` | deployment | active, executed and passing under `bin/deployment-gate.sh --offline`, which reached no provider. The packet is re-rendered from this candidate with the LP locker as the eighth predicted address, then `--prepare 0x9b2C414614aEE294202c1219520955EF3B596031` and `--rehearse` ran under the founder's separate read-only Base authority at observed block `51657720`: the live nonce (`0`), the mined salt, the eight predicted addresses (the prior seven unchanged) and the external control surface re-derived exactly, and the exact deployment script simulated cleanly with nothing broadcast |
...
- **A deployment or signed ceremony.** The fork gate is read-only; it deployed nothing to Base,
  ...
  eight predicted addresses and external observation were re-derived live by `--prepare` at block
  `51657720` and held by `--rehearse`, which simulated the exact script without a signer and
