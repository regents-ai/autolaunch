...
| Launcher REGENT | the launcher's wallet | any REGENT or allowance taken from a launcher for launching, which costs nothing |
| Swap fee REGENT or SUBJECT | PoolManager, hook, Regent Safe, splitter | the charged asset is not the realized unspecified currency, a lane is unfloored or misrouted, or inventory or allowance remains at the hook |
...
| Full-range LP position | PositionManager NFT owned forever by the strategy's immutable `RevstakeLPLocker`, which can only collect that position's fees into its fixed splitter | the position's principal becomes transferable or withdrawable by anyone, or its fees reach any destination other than its registered splitter |
...
4. **Governance.** The Regent Safe may pause and unpause new launches, and nothing else
   (`FAC-007`, `FAC-019`).
...
| --- | --- | --- | --- |
| Pause | governance Safe | `pauseLaunches`, `unpauseLaunches` | `FAC-007`, `FAC-019` |
| Launch | any caller, at no cost | `launch` | `FAC-020`, `FAC-027` |
...
| Swap fee settlement | PoolManager with a registered key | hook callbacks | `HOK-002`, `HOK-003`, `HOK-004` |
...
| PoolManager | pool initialization and swap settlement | only the bound singleton may invoke a hook callback and only a registered `PoolId` is served (`HOK-003`); settlement failure reverts the swap (`HOK-004`) |
| `SubjectSplitterV1` (per launch) | the hook's second 1% lane in the realized unspecified asset | the strategy binds only a splitter whose `regent`, `subject`, and `regentSafe` match the launch (`HOK-003`); an exact approval is fully consumed and the hook's charged-asset balance and allowance must return to their pre-callback levels, so a reverting, under-pulling, refunding, or re-entering splitter fails the whole swap (`HOK-002`, `HOK-004`) |
...
| REGENT, USDC, SUBJECT ERC20s | transfers, allowances | exact-allowance and exact-balance assertions (`FAC-027`, `MIG-016`) |
...
| Hook-mining error | a salt that produces a hook address whose permission bits differ from the three the hook declares, so v4 calls the wrong callbacks or none | `HOK-004` |
...
| Launch partial commit | failure at the token creation, either readback, the escrow approval, the escrow initialization or its exact pull, the strategy initialization or its exact pull, the auction creation or readback, the delivery, the custody proof, or the final record agreement, leaving a half-created launch | `FAC-021` |
| Launch cost | any REGENT pulled from a launcher, or any allowance a launch leaves standing | `FAC-027` |
...
| Protocol-account treasury abuse | naming the shared factory, the shared strategy, the bound hook, PoolManager, PositionManager, live staking, or the strategy's LP locker as a launch treasury, so a launch's payouts land in an account with no path back out or feed shared accounting that was never told about them. Those seven are the entire refusal set, judged as exact addresses | `STR-019`, `FAC-028` |
...
| Unreachable raise | a required raise of zero, or one above what the fixed ten-billion-SUBJECT auction can mathematically clear, guaranteeing a launch that can only fail | `STR-013`, `FAC-023` |
...
| Hook boundary escape | unregistered key, altered key field, foreign caller, hostile router | `HOK-003`, `HOK-004`, `HOK-007` |
| Official-pool pre-initialization | anyone bringing the deterministic official pool into existence at a price the auction never cleared, before or instead of the strategy | `HOK-003` |
| Hook permission forgery | a hook address carrying more, fewer, or different permission bits than the declared three, especially a pre-swap permission or a missing after-swap return-delta bit | `HOK-004` |
| Returned-delta manipulation | the wrong raw delta component or sign, a delta not backed by matching takes, an unsafe absolute value, or a two-lane total too large for the returned `int128` | `HOK-001`, `HOK-002`, `HOK-004` |
| Reentrancy | a recovery token, a hook callback, a receiver path, or a migration dependency re-entering mid-flow | `SPL-021`, `SPL-022`, `HOK-004`, `RCV-015`, `MIG-019` |
| Fee lane evasion | tiny swaps, realized rounding boundaries, partial or zero fills, exact output, either direction, or either currency ordering avoiding the lane implied by actual execution | `HOK-001`, `HOK-002`, `HOK-007`, `SPL-016` |
| Fee retention | hook keeping attributable REGENT or SUBJECT inventory or a splitter allowance for a later action | `HOK-002`, `HOK-004`, `HOK-006`, `INV-004` |
...
| Fees follow realized execution, including partial and zero fills | Both lanes are measured only after core execution from the absolute raw unspecified-currency delta. A price limit may fill part or none of the request; the specified-currency core result is unchanged and only the lane implied by actual execution is charged. Core may consume a few input units while producing zero output at a one-unit price movement, but a sub-100-unit realized fee base still floors both lanes to zero. | `HOK-001`, `HOK-002` |
| Exact-output take-before-settle is a liveness boundary | The after-swap fee takes are made before the router settles the trader's input. If the singleton PoolManager's existing balance of the realized input asset cannot cover both lanes, the whole exact-output swap reverts, even though later settlement would have replenished it. Nothing is half-paid: the Safe lane may be paid first and is rolled back with everything else. There is no deferral, queue, or retained inventory fallback. | `HOK-004` |
| Unrelated fee-asset gifts stay stuck at the hook | The hook has no recovery, sweep, receive, or fallback path. REGENT or SUBJECT sent to it outside a swap is not attributable fee inventory and is never spent, distributed, or recoverable; each swap finishes at exactly the pre-swap balance and allowance for its charged asset. Adding a rescue path would create new authority. | `HOK-002`, `HOK-006`, `INV-004` |
...
| The creator chooses the required raise, and the floor raise's economics were measured, not assumed | Any positive required raise up to `MAX_REACHABLE_RAISE` is admitted; there is no governance minimum. A one-wei raise is a creator's choice the design accepts, and the reference outcome — the floor raise of 10,000,000 REGENT, the whole auction allocation at the fixed floor price — really does resolve: measured against the real pinned auction and `PositionPlanner`, a single bid of exactly the floor raise sells the whole allocation, clears at the one price where that raise buys the whole allocation (`2^96 / 1000` rounded up, one Q96 unit above the rounded-down floor constant), migrates in both PoolKey orderings and at both bid-price endpoints — the floor price and the highest on-grid price — and the full-range position's exact REGENT and SUBJECT consumption is asserted. `NoFullRangePosition()` is not reachable inside the admitted range. | `FAC-023`, `STR-014` |
