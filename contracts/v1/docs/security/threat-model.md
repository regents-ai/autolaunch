...
| Unreachable or sub-floor raise | a required raise below the governance minimum (zero included), or one above what the fixed ten-billion-SUBJECT auction can mathematically clear, guaranteeing a launch that can only fail or a launch too small for the design | `STR-013`, `STR-020` |
...
| The floor required raise is admitted because it was measured, not assumed | The smallest admitted required raise is the strategy's governance minimum, born at 10,000,000 REGENT — the whole auction allocation at the fixed floor price — and the smallest reachable graduated outcome at that raise really does resolve: measured against the real pinned auction and `PositionPlanner`, a single bid of exactly the floor raise sells the whole allocation, clears at the one price where that raise buys the whole allocation (`2^96 / 1000` rounded up, one Q96 unit above the rounded-down floor constant), migrates in both PoolKey orderings and at both bid-price endpoints — the floor price and the highest on-grid price — and the full-range position's exact REGENT and SUBJECT consumption is asserted. `NoFullRangePosition()` is therefore not reachable inside the admitted range. Only the Governance and Regent Safe moves the floor, never below one wei or above `MAX_REACHABLE_RAISE`, and a change never rewrites an existing auction's terms. | `FAC-023`, `STR-014`, `STR-020` |
