...
#
#   SUPPLY-MANIFEST.txt   mix_lock_sha256, mix_cache_sha256, package_lock_sha256,
#                         esbuild-linux-<arch>.sha256, esbuild-linux-<arch>.file
#   MIX-CACHE.tar         the Mix, Hex and rebar3 caches, under a mix-cache/ prefix
#   npm-cache/            the sealed npm content cache (lands as npm-cache/_cacache/)
...
# The caches are keyed to one exact lockfile each. A drifted lockfile means the
# cache cannot satisfy an offline build, so stop before assembling anything.
