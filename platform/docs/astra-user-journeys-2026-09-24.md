...
Original implementation: `51cf78b` on `astra/user-journeys-2026-09-24`, based on `97f46b3`.
Integrated release branch: `fix/orv-ui`, merging `51cf78b` into `8a5d6ff`.
Build checkout: `/Users/sean/Documents/regent/worktrees/autolaunch/orv-ui`.
Use the resulting merge commit for the image; building the old `8a5d6ff` object excludes these changes.
...
The merge retains the SafeL2 allowance, production Base bidding fixes and explicit
site/chain origin flag from `8a5d6ff`. Public social filters and pagination now also
respect that site's-only default. Robinhood bids keep the release branch's "Won"
label only after bidding ends, together with the clearer settled-bid wording and
claim-to-staking links.

Both original additive migrations remain intact: `20260924134726_auction_activity`
and `20260924135016_launch_origin`. They were applied together to a fresh isolated
local database. Its columns and indexes were inspected. The regenerated combined
auction snapshot preserves `origin` plus all six activity fields and three indexes;
the generator's redundant duplicate activity migration was discarded, not applied.

Integration checks use only `autolaunch_astra_integration_0924_test` and
`autolaunch_astra_integration_checks_0924_test`. The former verified all eight social
filter combinations for auction/token pages, all 10,000 historical auctions, network
and type filters, cursor scope, ENS control/session checks, the confirmed claim
handoff, and confirmed activity persistence/replay/reorg notifications. Bid events
are inserted with the Ash bulk action and explicit upsert fields inside the existing
parent-lock transaction. Small helper extraction satisfies the release's code checks
without changing the user flow. No real wallet, OAuth, deployment or production data
was used for these integration checks.

The integrated `mix precommit` completed successfully (103 backend tests, format,
Credo, security scan, dependency checks, compile-reference check and Ash schema
check). TypeScript and the 12 JavaScript tests passed. Asset compilation passed.
No tests were added. The shared-source build-context script also passes shell syntax
validation; its ENS/SIWA inputs must be supplied when assembling the deploy image.
