defmodule AutolaunchWeb.Components.RaiseProgress do
  ...
  roughly when bidding opens or ends. The amounts and blocks are read from the
  ...
  def raise_progress(%{state: :active, block: block, end_block: end_block} = assigns)
      when block < end_block do
  ...
  def raise_progress(%{state: :created, block: block, start_block: start_block} = assigns)
      when block < start_block do
    ~H"""
  ...
  # From a thousand up, whole units with separators read best; the fraction is
  # dropped, never rounded up. Smaller amounts keep their significant digits.
  defp readable(amount) do
    decimal = Decimal.new(amount)

    if Decimal.gte?(decimal, 1000),
      do: decimal |> Decimal.round(0, :down) |> Decimal.to_string(:normal) |> Amounts.grouped(),
      else: amount
  end
