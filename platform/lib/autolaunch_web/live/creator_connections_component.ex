    ...
    socket =
      if socket.assigns[:scope] == scope,
        do: socket,
          ...
          |> assign(scope: scope, notice: nil, resolving: false)
          |> Phoenix.Component.assign(:ens_name, "")
          |> Phoenix.Component.assign(:ens_address, nil)
    ...
    {:ok,
     ...
     |> assign(
       ...
       if(socket.assigns.ens_name != "",
         ...
         else: (identities[:ens] && identities.ens.username) || ""
       )
     )
     |> assign(
       :ens_address,
       socket.assigns.ens_address || (identities[:ens] && identities.ens.metadata["wallet"])
     )}
  end
