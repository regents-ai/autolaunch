defmodule Autolaunch.Stocks.LaunchDraftImageStorage do
  @moduledoc """
  Stores immutable image versions and atomically selects one on the owner's
  Stocks launch draft.

  The same provenance model as `Autolaunch.LaunchDraftImageStorage` on the
  Stocks lane's own table and `/stock-images/...` URL shape: replacing the
  selected image never changes bytes behind a previously issued URL.
  """
  ...
  alias Autolaunch.Stocks.{LaunchDraft, LaunchDraftImage}
  ...
  defp store_transaction(expected, bytes, content_type, original_filename, digest, actor) do
    with {:ok, draft} <- locked_draft(expected.id, actor),
         ...
         {:ok, attached} <- Autolaunch.get_my_stocks_launch_draft_by_id(draft.id, actor: actor) do
  ...
  defp locked_draft(draft_id, actor) do
    case Autolaunch.get_my_stocks_launch_draft_for_update(draft_id, actor: actor) do
  ...
  defp existing_or_store(draft, bytes, content_type, original_filename, digest, actor) do
    case Autolaunch.get_my_stock_launch_draft_image_for_reuse(draft.id, digest, actor: actor) do
      {:ok, nil} ->
  ...
  defp create_image(draft, bytes, content_type, original_filename, actor) do
    case Autolaunch.create_stock_launch_draft_image(
           bytes,
           content_type,
           original_filename,
           draft.id,
  ...
  defp attach(draft, image, actor) do
    Autolaunch.attach_stocks_launch_draft_image(draft, image.id, actor: actor)
  end

  defp unchanged_selection(draft, expected) do
    if draft.stock_launch_draft_image_id == expected.stock_launch_draft_image_id,
      do: :ok,
